...
class MLP():
    ...
    def train(self, iters:int, lr:float, eps:float):
        self.history.append((0, self.or_w1, self.or_w2, self.or_b, self.nand_w1, self.nand_w2, self.nand_b, self.and_w1, self.and_w2, self.and_b))
        self.eps = eps
        self.lr = lr
        print_interval = iters // 10
        for i in range(iters):
            ...
            self.history.append((i+1, self.or_w1, self.or_w2, self.or_b, self.nand_w1, self.nand_w2, self.nand_b, self.and_w1, self.and_w2, self.and_b))
...
def plot_gate(gate_name, weight_cols, bias_col):
    fig, ax = plt.subplots(figsize=(8, 4))
    
    # Plot Weights
    for w_col in weight_cols:
        ax.plot(df["epoch"], df[w_col], label=w_col.split('_')[-1].upper())
    
    # Plot Bias (usually has a different scale/behavior)
    ax.plot(df["epoch"], df[bias_col], label="Bias", linestyle="--", color="black", alpha=0.7)
    
    ax.set_title(f"XOR Solution | {gate_name.upper()} Gate Evolution")
...
if __name__=="__main__": 
    ...
    model.train(iters=500, lr=.001, eps=.001)

    df = pd.DataFrame(model.history, columns=["epoch", "or_w1", "or_w2", "or_b", "nand_w1", "nand_w2", "nand_b", "and_w1", "and_w2", "and_b"])
    df.to_csv(f"{file_dir}/XOR_solution_train_history.csv", index=False)
    ...
    # 2. Generate the 3 Figures
    fig_or = plot_gate("OR", ["or_w1", "or_w2"], "or_b")
    fig_or.savefig(f"{file_dir}/XOR_solution_or_mse.png")
    fig_nand = plot_gate("NAND", ["nand_w1", "nand_w2"], "nand_b")
    fig_nand.savefig(f"{file_dir}/XOR_solution_nand_mse.png")
    fig_and = plot_gate("AND", ["and_w1", "and_w2"], "and_b")
    fig_and.savefig(f"{file_dir}/XOR_solution_and_mse.png")