    ...
    def train(self, training_dataset, iters, show_process='3'): # X_train should inclue y_train
        ...
        try:
            for i in range(iters):
                ...
                self.history.append((i, W.copy(), b, m2e))
...
if __name__=="__main__":
    ...
    try:
        ...
        ep_ver = input("Enter epochs and verbose level (e.g., 1000,2): ").split(',')
        if ep_ver == []:
            print("Enter epochs and verbose level (e.g., 1000,2): ")
            exit(0)
        epochs = int(ep_ver[0]) if len(ep_ver) > 0 else 10
        verbose = ep_ver[1] if len(ep_ver) > 1 else '2'
        ...
        df = pd.DataFrame(AI.history, columns=["epoch", "weights", "bias", "mse"])
        df.to_csv(f"{file_dir}/linear_regression_train_history.csv", index=False)
