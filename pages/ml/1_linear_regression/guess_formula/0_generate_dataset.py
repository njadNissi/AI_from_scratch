...
def nfeatures(features:int):
    ...
    dataset.to_csv(f'{file_dir}/nfFormulaData.csv', index=False)
...
def maths_magic(digits:int):
    ...
    print(dataset.head())
    file_dir = os.path.dirname(os.path.abspath(__file__)) + "/artifacts"
    dataset.to_csv(f'{file_dir}/mathsMagicData.csv', index=False)

    
if __name__=="__main__":
    
    print("Generating dataset for n-features linear regression...")
    nfeatures(features=3)
    print("Generating dataset for Maths Magic linear regression...")
