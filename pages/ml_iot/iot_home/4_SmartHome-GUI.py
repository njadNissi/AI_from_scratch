...
def AI(app:IoTHomeWindow, out_buffer:queue.Queue): 
    import pickle
    import numpy as np
    import pandas as pd
    ...
    while True:
        ...
        # Input collection
            ...
            values = (
                    f"{days[day_of_week]} {hour}h",
                    bool(occupancy), f"{ac_temp} °C",
                    light_on,
                    f"{room_temp} °C",
                    f"{out_temp} °C"
                )
            QUEUE.put(values) # MANUAL DATA
            print(values)
            time.sleep(1)
...
if __name__=="__main__":
    ...
    Thread(target=AI, args=(app, QUEUE), daemon=True).start()
